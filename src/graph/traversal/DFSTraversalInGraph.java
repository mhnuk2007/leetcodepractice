...
/**
 * Utility class for performing Depth-First Search (DFS) traversal on a graph.
 *
 * <p>DFS explores a graph by going as deep as possible along each branch
 ...
 * <p>Space Complexity: O(V) to store the visited array and the recursion stack.
 */
public class DFSTraversalInGraph {
    ...
    private DFSTraversalInGraph() {
    }

    /**
     * Main method to demonstrate DFS traversal with various test cases.
     *
     ...
     * @return a list of vertices in the order they were visited during DFS
     */
    public static List<Integer> traverseGraph(Graph graph, int v) {
        ...
        for (int i = 0; i < v; i++) {
            if (!visited[i]) {
                performDFS(graph, i, visited, result);
            }
        }
    ...
    /**
     * Internal helper method to perform DFS starting from a specific node.
     *
     * @param graph   the graph to traverse
     * @param node    the starting vertex for this DFS component
    ...
    private static void performDFS(Graph graph,
        ...
        for (int neighbor : graph.adj.get(node)) {
            if (!visited[neighbor]) {
                performDFS(graph, neighbor, visited, result);
            }
        }
    }
